...
from tbsim.utils.guidance_loss import verify_guidance_config_list, DiffuserGuidance, PerturbationGuidance
...
class CVAE(nn.Module):
    def __init__(
        ...
        self.current_perturbation_guidance = PerturbationGuidance(state_grad_general_transform, {'dt': 0.1})
        self.guidance_optimization_params = None

    def sample(self, condition_inputs, n: int, condition_feature=None, decoder_kwargs=None, with_guidance=False, batch_inputs=None):
        """
        ...
        if with_guidance:
            num_samp = z.shape[1]
            def decoder_wrapper(z_samples):
                return self.decoder(latents=z_samples, condition_features=c_samples, **decoder_kwargs)['trajectories']

            z_samples, _ = self.current_perturbation_guidance.perturb(z_samples, batch_inputs, self.guidance_optimization_params, num_samp=num_samp, decoder=decoder_wrapper)
    ...
    def sample_z(self, condition_inputs, n: int, condition_feature=None):
        if condition_feature is not None:
            c = condition_feature
        else:
            c = self.c_net(condition_inputs)  # [B, ...]
    ...
    def decode_z(self, z, cond_feat, decoder_kwargs=None):
        n = z.size(1)
        z_samples = TensorUtils.join_dimensions(z, begin_axis=0, end_axis=2)  # [B * N, ...]
        c_samples = TensorUtils.repeat_by_expand_at(cond_feat, repeats=n, dim=0)  # [B * N, ...]
        decoder_kwargs = dict() if decoder_kwargs is None else decoder_kwargs
        x_out = self.decoder(latents=z_samples, condition_features=c_samples, **decoder_kwargs)
        x_out = TensorUtils.reshape_dimensions(x_out, begin_axis=0, end_axis=1, target_dims=(cond_feat.shape[0], n))
        return x_out
    ...
    def set_guidance(self, guidance_config_list, example_batch=None):
        ...
        if guidance_config_list is not None:
            if len(guidance_config_list) > 0 and verify_guidance_config_list(guidance_config_list):
                ...
                # self.current_guidance = DiffuserGuidance(guidance_config_list)
