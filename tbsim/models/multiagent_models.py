...
class AgentAwareRasterizedModel(nn.Module):
    ...
    def forward_prediction(self, all_feats, data_batch, plan=None, with_guidance=False, data_batch_raw=None, num_samp=1, **kwargs):
        ...
        # make predictions
        # Only apply guidance to ego_agents trajectories (since they already include all actors)
        ego_preds = self.ego_decoder.forward(
            inputs=ego_feats, current_states=ego_states, with_guidance=with_guidance, data_batch=data_batch_raw, num_samp=num_samp)
    ...
    def set_guidance_optimization_params(self, guidance_optimization_params):
        self.ego_decoder.set_guidance_optimization_params(guidance_optimization_params)
        # self.agents_decoder.set_guidance_optimization_params(guidance_optimization_params)

    def set_guidance(self, guidance_config_list, example_batch=None):
        self.ego_decoder.set_guidance(guidance_config_list, example_batch)
        # self.agents_decoder.set_guidance(guidance_config_list)
    
    def clear_guidance(self):
        self.ego_decoder.clear_guidance()