...
class MATrafficModel(pl.LightningModule):
    ...
    def get_action(self, obs_dict, with_guidance=False, **kwargs):
            ...
            relevant_keys = ["curr_speed", "history_positions", "history_yaws", "extent","all_other_agents_history_extents",\
                "type","all_other_agents_types","all_other_agents_history_positions",\
                    "all_other_agents_history_yaws","history_availabilities",\
                        ...
                        "all_other_agents_history_availabilities", "all_other_agents_extents", "all_other_agents_curr_speed"]
            relevant_obs = {k: obs_dict[k] for k in relevant_keys}
            obs_tiled = TensorUtils.repeat_by_expand_at(relevant_obs, repeats=n, dim=0)
            # pass obs_dict directly for compatible data_batch with BC
            preds = self.model.forward_prediction(feats_tiled, obs_tiled, plan=plan_tiled, with_guidance=with_guidance, data_batch_raw=obs_dict, num_samp=n)
        else:
            plan = kwargs.get("plan", None)
            preds = self.model.forward_prediction(feats, obs_dict, plan, with_guidance=with_guidance, data_batch_raw=obs_dict, num_samp=n)

        ego_preds = self.model.get_ego_predictions(preds)

        ego_preds_positions = ego_preds["predictions"]["positions"]
        ego_preds_yaws = ego_preds["predictions"]["yaws"]
        ...
        if self.disable_control_on_stationary and self.stationary_mask is not None:
            BN = ego_preds_positions.shape[0]
            B = self.stationary_mask.shape[0]
            N = int(BN / B)
            stationary_mask_expand = self.stationary_mask.unsqueeze(1).expand(B, N).reshape(BN)
            ego_preds_positions[stationary_mask_expand] = 0
            ego_preds_yaws[stationary_mask_expand] = 0

        action = Action(
            positions=ego_preds_positions,
            yaws=ego_preds_yaws
        )
        return action, {}
    ...
    def set_guidance_optimization_params(self, guidance_optimization_params):
        self.model.set_guidance_optimization_params(guidance_optimization_params)

    def set_guidance(self, guidance_config_list, example_batch=None):
        self.model.set_guidance(guidance_config_list, example_batch)
