...
class EnvL5KitSimulation(BaseEnv, BatchedEnv):
    ...
    def _add_per_step_metrics(self, obs, actions=None):
        for k, v in self._metrics.items():
            if k.startswith("ego"):
                obs_cur = obs["ego"]
                action = actions.to_dict()['ego']
            elif k.startswith("agents"):
                obs_cur = obs["agents"]
                action = actions.to_dict()['agents']
            elif k.startswith("all"):
                obs_cur = self.combine_observations(obs)
                action = self.combine_observations(actions.to_dict())
            else:
                raise KeyError("Invalid metrics name {}".format(k))
            obs_cur['step_index'] = self._frame_index
            obs_cur['action'] = action
            v.add_step(obs_cur, self.current_scene_indices)
    ...
    def step(self, actions: RolloutAction, num_steps_to_take: int = 1, render=False):
        ...
        obs = self.get_observation()
        self._add_per_step_metrics(obs, actions)
